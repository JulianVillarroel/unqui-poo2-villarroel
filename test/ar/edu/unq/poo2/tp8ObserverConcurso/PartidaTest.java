...
class PartidaTest {
	...
	@Test
	void testLaPartidaNoComenzoYNoSePuedenVerificarPreguntas() {
		servidor.agregarParticipante(participante1);
	...
	void testElServidorRecibeUnaRespuestaDeUnJugadorYLaRespuestaEsCorrecta() {
		...
		when(participante1.getNombre()).thenReturn("Julian");
		servidor.iniciarPartida();
		servidor.verificarRespuesta(participante1, "a", "c");
		
		assertEquals(1, servidor.puntajeDeParticipante(participante1));
		verify(participante1).recibirNotificacionDeRespuesta("Correcta");
		verify(participante2).recibirNotificacion("Julian respondio correctamente la pregunta a");
		verify(participante3).recibirNotificacion("Julian respondio correctamente la pregunta a");
		verify(participante4).recibirNotificacion("Julian respondio correctamente la pregunta a");
		verify(participante5).recibirNotificacion("Julian respondio correctamente la pregunta a");
	}
	...
	void testElServidorTieneUnGanadorYFinalizaLaPartida() {
		...
		verify(participante1).recibirNotificacion("Julian gano la partida");
		verify(participante2).recibirNotificacion("Julian gano la partida");
		verify(participante3).recibirNotificacion("Julian gano la partida");
		verify(participante4).recibirNotificacion("Julian gano la partida");
		verify(participante5).recibirNotificacion("Julian gano la partida");
	}
